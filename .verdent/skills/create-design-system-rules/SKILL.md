...
Design system rules should be saved as project-level rule files that AI coding agents can automatically load. Common conventions include:

- A rules file in the project root (e.g., `RULES.md`, `DESIGN_SYSTEM.md`)
- A dedicated rules directory (e.g., `.rules/figma-design-system.md`)

Choose a location that fits your project's conventions. The key is that the rules are committed to version control and easily discoverable by the agent.
...
### Step 4: Save Rules to the Project

Save the generated rules to a project-level rule file so they are automatically loaded in future sessions.

**Recommended locations:**
- Project root: `DESIGN_SYSTEM_RULES.md` — simple, easy to find
- Dedicated directory: `.rules/figma-design-system.md` — better for projects with many rule files

If the project already has an existing rules file, append the design system rules as a new section.
